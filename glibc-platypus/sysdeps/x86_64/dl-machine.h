...
__attribute__((weak)) int8_t to_mitigate = 0;
...
{
  extern long long int _DYNAMIC[] attribute_hidden;
...
{
  uint8_t is_placeholder = 0;
  const char *strtab = (const char *) D_PTR(map, l_info[DT_STRTAB]);
  if (sym && strncmp(strtab + sym->st_name, "placeholder", 11) == 0) {
	is_placeholder = 1;
  }
...
# endif
	...
	case R_X86_64_GLOB_DAT:
	  if (is_placeholder) {
		for (int i = 0; i < 10; ++i) {
			if (placeholder_plts[i] == NULL) {
				//_dl_debug_printf("Reloc addr = 0x%lx\n", (unsigned long)reloc_addr);
				placeholder_plts[i] = map->l_addr + (void *)((uintptr_t)(*reloc_addr));
				//_dl_debug_printf("Populated with 0x%lx\n", (unsigned long) placeholder_plts[i]);
				//_dl_debug_printf("SSSSSSSYMBOL 0x%lx at %d\n", (unsigned long) placeholder_plts[i], placeholded);
				break;
			}
    	}
	  }
	  *reloc_addr = value;
	  //_dl_debug_printf("Got at 0x%lx with value 0x%lx\n", (unsigned long) reloc_addr, *reloc_addr);
	  break;
	// case R_X86_64_CALLBACK_TABLE: {
	// 	_dl_debug_printf("Relocating symbol: 0x%lx\n", sym_map->l_addr);

	// 	struct link_map *l = GL(dl_ns)[LM_ID_BASE]._ns_loaded;
	// 	*(Elf64_Addr *) reloc_addr = (Elf64_Addr) l->l_addr + l->l_info[DT_INIT]->d_un.d_ptr;
	// 	break;
	// }
	case R_X86_64_CALLBACK_TABLE:
	case R_X86_64_CALLBACK_MAYBE:
		//_dl_debug_printf("Populating with value 0x%lx, 0x%lx\n", reloc->r_addend + sym_map->l_addr, (unsigned long)reloc_addr);
		*reloc_addr = reloc->r_addend + sym_map->l_addr;
		//*reloc_addr = *(ElfW(Addr) *)value + reloc->r_addend;
		break;
...
# endif
	...
	case R_X86_64_ORMASK: {
	  //_dl_debug_printf("Now relocating or_mask with value 0x%lx\n", map->l_addr);
	  //_dl_debug_printf("MITIGATE IS %x\n", to_mitigate);
	  //struct link_map *l = GL(dl_ns)[LM_ID_BASE]._ns_loaded;
	  if (!to_mitigate) {
		*(Elf64_Addr *) reloc_addr = 0x0;
	  } else {
		*(Elf64_Addr *) reloc_addr = (Elf64_Addr) map->l_exec_start;
	  }
	//   if (l && l->l_info[DT_INIT]) {
	// 	_dl_debug_printf("Init addr is 0x%lx\n", l->l_addr + l->l_info[DT_INIT]->d_un.d_ptr);
	//   }
	  break;
	}
	case R_X86_64_ANDMASK: {
	  ...
	  //struct link_map *l = GL(dl_ns)[LM_ID_BASE]._ns_loaded;
	  if (!to_mitigate) {
		*(Elf64_Addr *) reloc_addr = 0xffffffffffffffff;
	  } else {
		*(Elf64_Addr *) reloc_addr = (Elf64_Addr) map->l_exec_end;
	  }
	  break;
	}
	case R_X86_64_64:
	case R_X86_64_DSO_BASE:
	case R_X86_64_EXPERIMENT:
