	     ...
	     struct link_map *skip, int type_class, struct link_map *undef_map)
{
  ...
  do
	...
	continue;
      ...
      /*
	if (undef_name && strcmp(undef_name, "callback_table") == 0) {
		_dl_debug_printf ("symbol=%s;  lookup in file=%s [%lu]\n",
				undef_name, DSO_FILENAME (map->l_name),
				map->l_ns);
	}
...
static struct link_map exp_map = {
    .l_addr = 0,
    .l_name = (char*)"cccallback_table",
    .l_ld = NULL,
    .l_next = NULL,
    .l_prev = NULL,
    // ... experiment ...
...
static struct link_map *mainbin[] = {NULL, NULL};
static struct r_scope_elem callback_stuff ={
	.r_list = mainbin,
	.r_nlist = 1,
};
...
static struct link_map fake_map = {0};
static ElfW(Sym) fake_sym = {0};
		     ...
		     int type_class, int flags, struct link_map *skip_map)
{
  ...
  if (placeholded) {
	for (int i = 0; i < placeholded; ++i) {
		if (strcmp(undef_name, placeholder_symbols[i]) == 0) {
			fake_map.l_addr = (ElfW(Addr))placeholder_plts[i];
			fake_sym.st_value = 0;
			*ref = &fake_sym;
			//_dl_debug_printf("Currently dlsyming sus symbol %s, populating with 0x%lx\n", placeholder_symbols[i], fake_map.l_addr);
			return &fake_map;
		}
	}
  }
  ...
  for (size_t start = i; *scope != NULL; start = 0, ++scope) {
	if (undef_name && strcmp(undef_name, "callback_table") == 0) {
		mainbin[0] = GL(dl_ns)[LM_ID_BASE]._ns_loaded;
		//_dl_debug_printf("SEARCHING FOR IT\n");
		if (do_lookup_x (undef_name, new_hash, &old_hash, *ref,
		     &current_value, &callback_stuff, 0, version, flags,
  ...
  if (__glibc_unlikely (current_value.s == NULL))
	...
	{
		if (undef_name && strcmp(undef_name, "callback_table") == 0) {
			_dl_debug_printf("CALLBACK TABLE NOT FOUND, RELOCATING WITH 0.\n");
			exp_map.l_addr = undef_map->l_addr;
			return &exp_map;
		}	
