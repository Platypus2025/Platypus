...
rtld_setup_main_map (struct link_map *main_map)
{
  ...
  for (const ElfW(Phdr) *ph = phdr; ph < &phdr[phnum]; ++ph)
	...
	if (ph->p_memsz > 0)
      ...
      //_dl_debug_printf("INITIMAGE IS 0x%lx\n", ph->p_vaddr);
  ...
  if (main_map->l_tls_initimage != NULL) {
    main_map->l_tls_initimage
      ...
      //_dl_debug_printf("2: INITIMAGE IS 0x%lx\n", (unsigned long) main_map->l_tls_initimage);
  }
  ...
  if (*user_entry == (ElfW(Addr)) ENTRY_POINT)
  ...
  else
    {
	 ...
	 This will be what dlopen on "" returns.  */
      ...
      main_map->l_exec_start = exec_start;
      main_map->l_exec_end = exec_end;
