...
URL="https://ftp.gnu.org/gnu/readline/${TARBALL}"
...
cp shlib/libreadline.so.8.3 "${LIB_FOLDER_UNINSTRUMENTED}/"
...
LOGFILE_PATH="$PWD/dynsym.log" make CC="${PLATYPUS_CLANG}" \
  CFLAGS="-g -fPIC -fpass-plugin=${DYNSYM_PLUGIN} -O3" \
...
LOGFILE_PATH="$PWD/sym.log" make CC="${PLATYPUS_CLANG}" \
...
PROTECT_JMP=True make CC=${PLATYPUS_CLANG} \
  CFLAGS="-fPIC -g \
          -include ${PWD}/../masks.h \
          -O3 \
          -fcf-protection=full \
  ...
  LDFLAGS="-fuse-ld=lld \
           ...
           -Wl,--strip-debug \
           -Wl,--allow-shlib-undefined -Wl,--callb_getter=${PWD}/../a.txt" \
  -j8
