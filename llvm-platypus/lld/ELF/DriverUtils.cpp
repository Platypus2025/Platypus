...
opt::InputArgList ELFOptTable::parse(Ctx &ctx, ArrayRef<const char *> argv) {
  ...
  for (opt::Arg *arg : args.filtered(OPT_UNKNOWN)) {
    ...
    /* HERE WE CHECK THE CUSTOM LINKER ARGUMENT */
    std::string custom_arg = arg->getAsString(args);
    auto eq_pos = custom_arg.find('=');
    bool custom_prefix = (eq_pos != std::string::npos) &&
                      (custom_arg.substr(0, eq_pos) == "--callb_getter");
    if (custom_prefix) {
      std::string filePath = custom_arg.substr(eq_pos + 1);
      std::ifstream symfile(filePath);
      if (!symfile) {
          ErrAlways(ctx) << "Missing file " << filePath;
          return args;
      }
      std::string line;
      while (std::getline(symfile, line)) {
          if (!line.empty()) { // Optional: skip empty lines
            auto pos = line.find(':');
            if (pos == std::string::npos) exit(1337);
            std::string key = line.substr(0, pos);
            std::string values = line.substr(pos + 1);
            if (values.size() < 2 || values.front() != '(' || values.back() != ')') exit(1337);
            ...
            while (std::getline(ss, item, ',')) {
                //item = trim(item);
                if (!item.empty())
                    arg_pos.push_back(std::stoi(item));
            }
            ctx.callb_get[key] = arg_pos;
            //ctx.callb_get.push_back(std::make_tuple(line));
          }
      }
      continue;
    }
