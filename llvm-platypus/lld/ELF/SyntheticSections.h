...
public:
  ...
  void computeRaw(Ctx &, SymbolTableBaseSection *symt, bool reladyn = false);
...
/*
 *  This structs holds for each symbol that gets a PLT entry (fake or no)
 *  apart from the symbol itself, the number of the previous Plt symbols
 *  tha are non FAKE-PLT.
 */

// struct PltMappings {
//   const Symbol *sym;
//   uint64_t non_fake_plt;
// };
...
public:
  ...
  /* 
   * This variable holds how many of the plt entries require also a fake one.
   * This is needed for easy indexing of the .got.plt entries.
   */
  uint32_t bothplts = 0;
};

/* HERE WE DEFINE THE FAKE PLT SECTION */
class FakePltSection : public SyntheticSection {
  public:
    FakePltSection(Ctx &);
    void writeTo(uint8_t *buf) override;
    size_t getSize() const override;
...
class FakeGotPltSection final : public SyntheticSection {
  public:
    FakeGotPltSection(Ctx &);
    void addEntry(Symbol &sym);
    size_t getSize() const override;
    void writeTo(uint8_t *buf) override;
    bool isNeeded() const override;
