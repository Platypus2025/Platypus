...
void GotPltSection::addEntry(Symbol &sym) {
  assert(sym.auxIdx == ctx.symAux.size() - 1);
  //assert(sym.auxIdx == ctx.symAux.size() - 1 &&
         //(ctx.symAux.back().pltIdx + ctx.symAux.back().fakepltIdx) == entries.size() - 1);
...
bool customRelocComparator(const Symbol* a, const Symbol* b) {
  return (a->customreloc_type() & 0x1) < (b->customreloc_type() & 0x1);
}

void GotPltSection::writeTo(uint8_t *buf) {
  ctx.target->writeGotPltHeader(buf);
  buf += ctx.target->gotPltHeaderEntriesNum * ctx.target->gotEntrySize;
  //std::sort(entries.begin(), entries.end(), customRelocComparator);
...
uint32_t DynamicReloc::getSymIndex(SymbolTableBaseSection *symTab) const {
  ...
  if (sym->getName().starts_with("or_mask") || sym->getName().starts_with("and_mask")) {
    return 0;
  }
...
void RelocationBaseSection::finalizeContents() {
  ...
  if (symTab && symTab->getParent()) {
    ...
    //printf("Finalizing %s, %d\n", this->name, symTab->getParent()->sectionIndex);
  } else
    getParent()->link = 0;
...
void DynamicReloc::computeRaw(Ctx &ctx, SymbolTableBaseSection *symt, bool reladyn) {
  r_offset = getOffset();
  
  if (reladyn && this->sym && (this->sym->customreloc_type() & 0x1)) {
    bool found = false;
    size_t pltOffset = 0;//ctx.target->pltEntrySize;
    for (const Symbol *symb : ctx.in.fakeplt->entries) {
      if (symb->getName() == this->sym->getName()) {
          found = true;
          break;
      }
      pltOffset += ctx.target->pltEntrySize;
    }
    if (found) {
      addend = pltOffset + ctx.in.fakeplt->getVA();
      r_sym = 0;
      //printf("RSYM = %lx, %lx, %lx\n", r_sym, r_offset, addend);
    } else {
      printf("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW\n");
      exit(1337);
    }
  } else if (reladyn && this->sym && (this->sym->customreloc_type() == 2)) {
    addend = 0;
    r_sym = getSymIndex(symt);
    printf("FOR ORMAsk\n");
    if (this->sym->getName().starts_with("callback_table") || this->sym->getName().starts_with("placeholder") || this->sym->getName().starts_with("dsym_")) {
      r_sym = getSymIndex(symt);
    }
  } else if (reladyn && this->sym && (this->sym->customreloc_type() == 4)) {
    addend = (this->sym->iscustomreloc & 0xffffff00) >> 8;
    r_sym = getSymIndex(symt);
  } else if (reladyn && this->sym && (this->sym->customreloc_type() == 6)) {
    addend = 0;
    //r_sym = getSymIndex(symt);
    //printf("asasdfasdfasdfa\n");
  } else if (reladyn && this->sym && this->sym->getName().starts_with("_DYNAMIC")) {
    //printf("AAAAAAAAAAAAAAAAA %s\n", sym.getName());
    //type = R_X86_64_DSO_BASE;
    r_sym = getSymIndex(symt);
    addend = 0;
    //sym.iscustomreloc = 0x6;
  } else if (reladyn && this->sym && (this->sym->customreloc_type() == 10)) {
    addend = 0;
    r_sym = getSymIndex(symt);
    printf("FOR ANDMAsk\n");
  } else {
...
void PltSection::addEntry(Symbol &sym) {
  ...
  if (!(sym.customreloc_type() & 0x1)) {
    ctx.symAux.back().non_fake_plt = entries.size();
    ctx.symAux.back().both_plts = bothplts;
  } else {
    bothplts += 1;
  }
  ctx.symAux.back().pltIdx = entries.size() + ctx.in.fakeplt->getNumEntries();
  entries.push_back(&sym);
}

size_t PltSection::getSize() const {
  return headerSize + entries.size() * ctx.target->pltEntrySize; //+ 0x1000;
}
...
/* 
 * HERE WE DEFINE THE FAKE PLT FUNCTIONS REGARDING fakeplt and fakegotplt sections
 *
 * We use the same constants-variables (e.g. pltHeaderSize) for simplicity
 */

FakePltSection::FakePltSection(Ctx &ctx)
    : SyntheticSection(ctx, ".fakeplt", SHT_PROGBITS, SHF_ALLOC | SHF_EXECINSTR,
                       16),
      headerSize(0) {
  // On PowerPC, this section contains lazy symbol resolvers.
  if (ctx.arg.emachine == EM_PPC64) {
...
void FakePltSection::writeTo(uint8_t *buf) {
  // At beginning of PLT, we have code to call the dynamic
  // linker to resolve dynsyms at runtime. Write such code.
  
  //ctx.target->writeFakePltHeader(buf);
  size_t off = headerSize;

  for (const Symbol *sym : entries) {
    //printf("PLT AT %lx\n", getVA());
    ctx.target->writeFakePlt(buf + off, *sym, getVA() + off);
    off += ctx.target->pltEntrySize;
  }
}

void FakePltSection::addEntry(Symbol &sym) {
  assert(sym.auxIdx == ctx.symAux.size() - 1);
  ctx.symAux.back().non_fake_plt = entries.size();
  ctx.symAux.back().both_plts = ctx.in.plt->bothplts;
  ctx.symAux.back().fakepltIdx = entries.size() + ctx.in.plt->getNumEntries();
  entries.push_back(&sym);
}

size_t FakePltSection::getSize() const {
  return headerSize + entries.size() * ctx.target->pltEntrySize;
}

bool FakePltSection::isNeeded() const {
  // For -z retpolineplt, .iplt needs the .plt header.
  return !entries.empty() || (ctx.arg.zRetpolineplt && ctx.in.iplt->isNeeded());
}

void FakePltSection::addSymbols() {
  ctx.target->addPltHeaderSymbols(*this);
...
FakeGotPltSection::FakeGotPltSection(Ctx &ctx)
    : SyntheticSection(ctx, ".fakegot.plt", SHT_PROGBITS, SHF_ALLOC | SHF_WRITE,
                       ctx.arg.wordsize) {
  if (ctx.arg.emachine == EM_PPC) {
    name = ".plt";
  } else if (ctx.arg.emachine == EM_PPC64) {
...
void FakeGotPltSection::addEntry(Symbol &sym) {
  assert(sym.auxIdx == ctx.symAux.size() - 1 &&
         ctx.symAux.back().fakepltIdx == entries.size());
  entries.push_back(&sym);
}

size_t FakeGotPltSection::getSize() const {
  return (ctx.target->gotPltHeaderEntriesNum + entries.size()) *
         ctx.target->gotEntrySize;
}

void FakeGotPltSection::writeTo(uint8_t *buf) {
  ctx.target->writeGotPltHeader(buf);
  buf += ctx.target->gotPltHeaderEntriesNum * ctx.target->gotEntrySize;
  for (const Symbol *b : entries) {
    ctx.target->writeGotPlt(buf, *b);
...
bool FakeGotPltSection::isNeeded() const {
  // We need to emit GOTPLT even if it's empty if there's a relocation relative
  // to it.
  return !entries.empty() || hasGotPltOffRel;
}
...
template <class ELFT> void elf::createSyntheticSections(Ctx &ctx) {
  ...
  add(*ctx.in.gotPlt);
  //ctx.in.fakegotPlt = std::make_unique<FakeGotPltSection>(ctx);
  //add(*ctx.in.fakegotPlt);
  ...
  if (ctx.sym.globalOffsetTable && ctx.arg.emachine != EM_MIPS) {
    if (ctx.target->gotBaseSymInGotPlt) {
      ...
      //ctx.in.fakegotPlt->hasGotPltOffRel = true;
    }
    else
  ...
  if (ctx.arg.emachine == EM_PPC)
    ...
    ctx.in.fakeplt = std::make_unique<FakePltSection>(ctx);
  }
  //ctx.in.plt->addralign = std::max<uint64_t>(ctx.in.plt->addralign, 0x1000);
  add(*ctx.in.plt);
  add(*ctx.in.fakeplt);
