...
void OutputSection::commitSection(InputSection *isec) {
  //printf("Currently at input section %s\n", isec->name);
  ...
  auto fn = [=, &ctx](size_t begin, size_t end) {
    ...
    for (size_t i = begin; i != end; ++i) {
      ...
      if (nonZeroFiller) {
        // if (name == ".plt") {
        //   printf("Name %s\n", isec->getParent()->name);
        //   printf("Plt size is %lx, %lx\n", isec->getSize(), isec->outSecOff);
        // }
        ...
        // if (isec->name == ".plt") {
        //   printf("Start at: %lx, End at: %lx, Diff is: %lx\n", *start, *end, *end - *start);
        // }
        if (isec->nopFiller) {
