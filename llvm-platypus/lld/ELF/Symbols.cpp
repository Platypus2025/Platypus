...
uint64_t Symbol::getGotPltVA(Ctx &ctx) const {
  ...
  // printf("At address: %lx\n\n", ctx.in.gotPlt->getVA() + getGotPltOffset(ctx));
...
uint64_t Symbol::getGotPltOffset(Ctx &ctx) const {
  ...
  //printf("PLTIDX IS %lx and name is %s\n", getPltIdx(ctx), getName());
  if (customreloc_type() == 0x1) {
    //printf("Offset is %lx\n", (getFakePltIdx(ctx) + ctx.in.plt->getNumEntries() + ctx.target->gotPltHeaderEntriesNum) * ctx.target->gotEntrySize);
    return (getFakePltIdx(ctx) - removeDuplicatePlts(ctx) + ctx.target->gotPltHeaderEntriesNum) *
         ctx.target->gotEntrySize;
  }
  // printf("Symbol %s\n", this->getName());
  // printf("Take in %lx\n\n", (getPltIdx(ctx) + ctx.target->gotPltHeaderEntriesNum) *
  // ctx.target->gotEntrySize);
  return (getPltIdx(ctx) - removeDuplicatePlts(ctx) + ctx.target->gotPltHeaderEntriesNum) *
         ctx.target->gotEntrySize;
}
...
uint64_t Symbol::getPltVA(Ctx &ctx) const {
  uint64_t outVA = isInIplt ? ctx.in.iplt->getVA() +
                            ...
                            : ctx.in.plt->getVA() + ctx.in.plt->headerSize +
                                  (getCorrectedPltIdx(ctx)) * ctx.target->pltEntrySize;
                            // : ctx.in.plt->getVA() + ctx.in.plt->headerSize +
...
uint64_t Symbol::getFakePltVA(Ctx &ctx) const {
  uint64_t outVA = isInIplt ? ctx.in.iplt->getVA() +
                                  getFakePltIdx(ctx) * ctx.target->ipltEntrySize
                              : ctx.in.fakeplt->getVA() + ctx.in.fakeplt->headerSize +
                                    (getCorrectedPltIdx(ctx)) * ctx.target->pltEntrySize;
                            // : ctx.in.fakeplt->getVA() + ctx.in.fakeplt->headerSize +
                            //       (getFakePltIdx(ctx)) * ctx.target->pltEntrySize;
  ...
  if (ctx.arg.emachine == EM_MIPS && isMicroMips(ctx))
    outVA |= 1;
  //printf("OULALAL %lx %lx %lx %lx\n", ctx.in.fakeplt->getVA(), getCorrectedPltIdx(ctx), getFakePltIdx(ctx), outVA);
  return outVA;
} 
uint64_t Symbol::getFakeGotPltOffset(Ctx &ctx) const {
  if (isInIplt)
    return getFakePltIdx(ctx) * ctx.target->gotEntrySize;
  return (getFakePltIdx(ctx) + ctx.target->gotPltHeaderEntriesNum) *
         ctx.target->gotEntrySize;
}

uint64_t Symbol::getFakeGotPltVA(Ctx &ctx) const {
  if (isInIplt)
    return ctx.in.igotPlt->getVA() + getGotPltOffset(ctx);
  return ctx.in.fakegotPlt->getVA() + getFakeGotPltOffset(ctx);
}
...
void elf::parseVersionAndComputeIsPreemptible(Ctx &ctx) {
  ...
  for (Symbol *sym : ctx.symtab->getSymbols()) {
    ...
    if (!sym->isDefined() && !sym->isCommon()) {
      ...
      // if (sym->getName() == "puts" && !sym->isPreemptible) {
      //   printf("FIN\n");
      // }
...
void Symbol::resolve(Ctx &ctx, const Defined &other) {
  ...
  if (shouldReplace(ctx, other)) {
    // if (this->getName() == "TASN") {
    //   printf("Sym Vis: %lx, Other Vis: %lx\n", isExported, other.isExported);
    //   return;
    // }
    other.overwrite(*this);
  }
