...
void ObjFile<ELFT>::initializeSections(bool ignoreComdats,
  ...
  bool mainBin = false;
  if (!this->getName().empty() && this->getName()[0] != '/') {
    mainBin = true;
  }
  for (size_t i = 0; i != size; ++i) {
  ...
  for (size_t i = 0; i != size; ++i) {
    if (this->sections[i] == &InputSection::discarded)
      continue;
    const Elf_Shdr &sec = objSections[i];
    if (mainBin && CHECK2(obj.getSectionName(sec), this) == ".rela.text") {
      //printf("Let's dump .rela.txt\n");
      mainBin = false;
      /*
      auto relasOrErr = obj.template getSectionContentsAsArray<typename ELFT::Rela>(sec);
      if (!relasOrErr) {
          printf("Failed to get relocations\n");
          return;
      }

      // Convert the const ArrayRef to a mutable vector
      std::vector<typename ELFT::Rela> relas(relasOrErr->begin(), relasOrErr->end());

      for (auto &rela : relas) {
          printf("Offset: 0x%lx, Symbol: %d, Type: %lx\n",
                rela.r_offset, rela.r_addend, rela.r_info & 0xffffffffL);
          rela.r_info &= 0xffffff00L;
          rela.r_info |= 0xffffff04L;
      }
          */
    }
...
InputSectionBase *ObjFile<ELFT>::createInputSection(uint32_t idx,
  ...
  if (name == ".lel") {
    printf("%llu %llu %llu %llu %llu %llu %llu\n", sec.sh_name, sec.sh_type, sec.sh_flags, sec.sh_addr, sec.sh_offset, sec.sh_size, sec.sh_addralign);
  }
...
void ObjFile<ELFT>::initializeSymbols(const object::ELFFile<ELFT> &obj) {
  ...
  for (unsigned i : undefineds) {
    ...
    sym->isUsedInRegularObj = true;
    sym->referenced = true;
    // if (symbols[i]->getName() == "puts" && !symbols[i]->isPreemptible) {
    //   printf("FINALLY\n");
    // }
  }
}
