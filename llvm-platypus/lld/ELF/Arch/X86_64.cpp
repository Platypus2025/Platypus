...
public:
  ...
  void writeFakePltHeader(uint8_t *buf) const override;
  void writeFakePlt(uint8_t *buf, const Symbol &sym,
                uint64_t pltEntryAddr) const override;  
...
X86_64::X86_64(Ctx &ctx) : TargetInfo(ctx) {
  ...
  customRel = R_X86_64_RELATIVE;
  ormaskRel = R_X86_64_ORMASK;
  andmaskRel = R_X86_64_ANDMASK;
  callbackRel = R_X86_64_CALLBACK_TABLE;
  callbackmaybeRel = R_X86_64_CALLBACK_MAYBE;
  dsobaseRel = R_X86_64_DSO_BASE;
...
RelExpr X86_64::getRelExpr(RelType type, const Symbol &s,
  ...
  case R_X86_64_32S:
  case R_X86_64_64:
  case R_X86_64_ORMASK:
  case R_X86_64_ANDMASK:
  case R_X86_64_CALLBACK_TABLE:
  case R_X86_64_CALLBACK_MAYBE:
  case R_X86_64_DSO_BASE:
    return R_ABS;
...
void X86_64::writeGotPlt(uint8_t *buf, const Symbol &s) const {
  ...
  if (ctx.callb_get.count(s.getName().str())) {
    if (s.customreloc_type() & 0x1) {
      write64le(buf, s.getFakePltVA(ctx) + 6 + 2 + 2);
    } else {
      write64le(buf, s.getPltVA(ctx) + 6 + 2 + 2);
    }
  } else {
    if (s.customreloc_type() & 0x1) {
      write64le(buf, s.getFakePltVA(ctx) + 6);
    } else {
...
void X86_64::writePltHeader(uint8_t *buf) const {
  ...
  /* 
   * Take into account that maybe a symbol requires both fake and normal Plt.
   * In this case, just use the .got.plt value from the fake one and also the
   * same symbol relocation.
   */

const uint8_t callback_reg[6] = {0x3d, 0x35, 0x15, 0x0d, 0x05, 0x0d};

void X86_64::writePlt(uint8_t *buf, const Symbol &sym,
                      uint64_t pltEntryAddr) const {

  if (ctx.callb_get.count(sym.getName().str())) {
    std::vector<int> arg_pos = ctx.callb_get[sym.getName().str()];
    uint8_t init = 0x0;
    uint8_t reg = 0x0;

    if (arg_pos.size() != 1) exit(0xdead);
    if (arg_pos[0] < 4) {
      init = 0x48;
    } else if (arg_pos[0] == 4 || arg_pos[0] == 5){
      init = 0x4c;
    } else {
      exit(0xdead);
    }
    reg = callback_reg[arg_pos[0]];
    
    const uint8_t inst[] = {
      //0x48, 0x0f, 0x6e, 0xec,
      init, 0x0b, reg, 0, 0, 0, 0,
    ...
    write32le(buf + 3, ctx.ormask_offset + ctx.in.got->getVA() - pltEntryAddr - 7);
    write32le(buf + 2 + 7, sym.getGotPltVA(ctx) - pltEntryAddr - 6 - 7);
    if (sym.customreloc_type() & 0x1) {
      assert(sym.getFakePltIdx(ctx) >= 0);
      write32le(buf + 7 + 7, sym.getFakePltIdx(ctx) - sym.removeDuplicatePlts(ctx));
    } else {
      write32le(buf + 7 + 7, sym.getPltIdx(ctx) - sym.removeDuplicatePlts(ctx));
    }
    write32le(buf + 12 + 7, ctx.in.plt->getVA() - pltEntryAddr - 16 - 7);
  } else {
    const uint8_t inst[] = {
        0xff, 0x25, 0, 0, 0, 0, // jmpq *got(%rip)
        0x68, 0, 0, 0, 0,       // pushq <relocation index>
    ...
    if (sym.customreloc_type() & 0x1) {
      assert(sym.getFakePltIdx(ctx) >= 0);
      write32le(buf + 7, sym.getFakePltIdx(ctx) - sym.removeDuplicatePlts(ctx));
    } else {
      write32le(buf + 7, sym.getPltIdx(ctx) - sym.removeDuplicatePlts(ctx));
    }
    write32le(buf + 12, ctx.in.plt->getVA() - pltEntryAddr - 16);
  }
  // memcpy(buf, inst, sizeof(inst));
  // write32le(buf + 2 + 2 + 2, sym.getGotPltVA(ctx) - pltEntryAddr - 6 -2 - 2);
  // //write32le(buf + 7, sym.getPltIdx(ctx) - sym.removeDuplicatePlts(ctx));
  // if (sym.customreloc_type() & 0x1) {
  //   assert(sym.getFakePltIdx(ctx) >= 0);
  //   write32le(buf + 7 + 2 + 2, sym.getFakePltIdx(ctx) - sym.removeDuplicatePlts(ctx));
  //   //printf("PltIdx is: %lx, FakepltIdx is: %lx, gotpltva is : %lx\n", sym.getPltIdx(ctx), sym.getFakePltIdx(ctx), sym.getGotPltVA(ctx));
  // } else {
  //   write32le(buf + 7 + 2 + 2, sym.getPltIdx(ctx) - sym.removeDuplicatePlts(ctx));
  // }
  // write32le(buf + 12 + 2 + 2, ctx.in.plt->getVA() - pltEntryAddr - 16 - 2 - 2);
}

/* ADD OUR FAKE PLT STUFF */
void X86_64::writeFakePltHeader(uint8_t *buf) const {
  const uint8_t pltData[] = {
      0xff, 0x35, 0, 0, 0, 0, // pushq GOTPLT+8(%rip)
      0xff, 0x25, 0, 0, 0, 0, // jmp *GOTPLT+16(%rip)
      0x0f, 0x1f, 0x40, 0x00, // nop
...
void X86_64::writeFakePlt(uint8_t *buf, const Symbol &sym,
                        uint64_t pltEntryAddr) const {
  ...
  if (ctx.callb_get.count(sym.getName().str())) {
    ...
    const uint8_t Inst[] = {
        ...
        init, 0x0b, reg, 0, 0, 0, 0,  // or   reg, ormask
        init, 0x23, reg, 0, 0, 0, 0,  // and  reg, andmask     
        0xff, 0x25, 0,    0,    0, 0, // jmpq *got(%rip)
        0x90, 0x90,        // nop hlt
    };

    memcpy(buf, Inst, sizeof(Inst));
    write32le(buf + 4 + 3, ctx.ormask_offset + ctx.in.got->getVA() - pltEntryAddr - 4 - 7);
    write32le(buf + 4 + 7 + 3, ctx.andmask_offset + ctx.in.got->getVA() - pltEntryAddr - 4 - 7 - 7);
    write32le(buf + 6 + 7 + 7, sym.getGotPltVA(ctx) - pltEntryAddr - 10 - 7 - 7);
...
//     assert(sym.customreloc_type() & 0x1);
...
RelType X86_64::getDynRel(RelType type) const {
  if (type == R_X86_64_64 || type == R_X86_64_PC64 || type == R_X86_64_SIZE32 ||
      type == R_X86_64_SIZE64 || type == R_X86_64_CALLBACK_MAYBE)
    return type;
...
void X86_64::relocateAlloc(InputSectionBase &sec, uint8_t *buf) const {
  ...
  for (const Relocation &rel : sec.relocs()) {
    ...
    // if (rel.sym->iscustomreloc || rel.sym->getName().starts_with("or_mask")) {
    //   printf("Reloc %lx %lx %lx\n", val, rel.offset, loc);
    // }
...
void IntelIBT::writeGotPlt(uint8_t *buf, const Symbol &s) const {
  uint64_t va;
  if (s.getName().starts_with("placeholder")) {
    printf("Idx is 0x%lx\n",s.getFakePltVA(ctx));
    va = s.getFakePltVA(ctx);
  } else {
    va = ctx.in.ibtPlt->getVA() + IBTPltHeaderSize +
...
void IntelIBT::writePlt(uint8_t *buf, const Symbol &sym,
    ...
    if (ctx.callb_get.count(sym.getName().str())) {
      ...
      memcpy(buf, Inst, sizeof(Inst));
      if (sym.getName().starts_with("placeholder")) {
        write32le(buf + 6, pltEntryAddr);
      } else {
        write32le(buf + 6, sym.getGotPltVA(ctx) - pltEntryAddr - 10);
      }
    }
