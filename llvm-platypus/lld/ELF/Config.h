...
struct SymbolAux {
  uint32_t gotIdx = -1;
  uint32_t pltIdx = -1;
  uint32_t fakepltIdx = -1;
  uint32_t tlsDescIdx = -1;
  uint32_t tlsGdIdx = -1;
  uint32_t non_fake_plt = 0;
  uint32_t both_plts = 0;
};
...
struct InStruct {
  ...
  std::unique_ptr<FakeGotPltSection> fakegotPlt;
  ...
  std::unique_ptr<FakePltSection> fakeplt;
...
struct Ctx : CommonLinkerContext {
  ...
  uint64_t ormask_offset = 0;
  uint64_t andmask_offset = 0;
  ...
  /* CUSTOM DICT TO HOLD CALLBACK GETTERS */
  //std::vector<std::any> callb_get;
  std::map<std::string, std::vector<int>> callb_get;
  //std::unordered_set<std::string> callb_get;
  std::vector<std::string> sym_renames;
