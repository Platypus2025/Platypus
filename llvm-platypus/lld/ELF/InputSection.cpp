...
uint64_t InputSectionBase::getRelocTargetVA(Ctx &ctx, const Relocation &r,
  ...
  case R_PLT: {
    if (r.sym->customreloc_type() & 0x1) {
      return r.sym->getFakePltVA(ctx) + a;
    } else {
  ...
  case RE_PPC64_CALL_PLT: {
    if (r.sym->customreloc_type() & 0x1) {
      return r.sym->getFakePltVA(ctx) + a - p;
    } else {
  ...
  case R_PLT_GOTPLT: {
    if (r.sym->customreloc_type() & 0x1) {
      return r.sym->getFakePltVA(ctx) + a - ctx.in.gotPlt->getVA();
    } else {
  ...
  case R_PLT_GOTREL: {
    if (r.sym->customreloc_type() & 0x1) {
      return r.sym->getFakePltVA(ctx) + a - ctx.in.got->getVA();
    } else {
  ...
  default:
    printf("UNreachable 0x%lx %s\n", r.expr, r.sym->getName().str().c_str());
