...
public:
  ...
  /* CUSTOM FIELD FOR OUR RELOC TYPE */
  uint32_t iscustomreloc = 0x0;
  uint32_t customreloc_type() const {return (uint8_t)(iscustomreloc & 0xFF);}
  ...
  void rename(uint32_t len) {
    auto newName = getName().substr(0, len);
    setName(newName);
    // char *ren = (char *)nameData;
    // printf("nameData pointer: %p\n", (void*)ren);
    // printf("nameData string: '%s'\n", ren);
    // printf("Len is 0x%lx\n", len);
    // ren[len] = '\0';
  }
  ...
  uint32_t getFakePltIdx(Ctx &ctx) const { return ctx.symAux[auxIdx].fakepltIdx; }
  uint32_t getCorrectedPltIdx(Ctx &ctx) const {return ctx.symAux[auxIdx].non_fake_plt; }
  uint32_t removeDuplicatePlts(Ctx &ctx) const {return ctx.symAux[auxIdx].both_plts; }
  uint64_t getFakeGotPltOffset(Ctx &) const;
  ...
  uint64_t getFakeGotPltVA(Ctx &) const;
  uint64_t getPltVA(Ctx &) const;
  uint64_t getFakePltVA(Ctx &) const;
