...
static void addPltEntry(Ctx &ctx, PltSection &plt, GotPltSection &gotPlt,
  ...
  //printf("Creating PLT for: %s\n", sym.getName());//with type: %d\n", sym.getName(), type);
  plt.addEntry(sym);
  if (!(sym.customreloc_type() & 0x1)) {
    gotPlt.addEntry(sym);
...
/* OUR FAKE PLT ENTRY */
template <class FakePltSection, class GotPltSection>
static void addFakePltEntry(Ctx &ctx, FakePltSection &fakeplt, GotPltSection &gotPlt,
                        RelocationBaseSection &rel, RelType type, Symbol &sym) {
  //printf("Creating FAKEPLT for: %s\n", sym.getName());//with type: %d\n", sym.getName(), type);
  fakeplt.addEntry(sym);
  gotPlt.addEntry(sym);
  rel.addReloc({type, &gotPlt, sym.getGotPltOffset(ctx),
                sym.isPreemptible ? DynamicReloc::AgainstSymbol
                                  : DynamicReloc::AddendOnlyWithTargetVA,
...
void elf::addGotEntry(Ctx &ctx, Symbol &sym) {
  ...
  if (sym.customreloc_type() > 1) {
    if (sym.getName().starts_with("or_mask")) {
      if (ctx.ormask_offset == 0)
        ctx.ormask_offset = off;
      ctx.mainPart->relaDyn->addReloc({ctx.target->ormaskRel, ctx.in.got.get(), off,
        DynamicReloc::AgainstSymbol, sym, 0,
        R_ABS});
        return;
    } else if (sym.getName().starts_with("and_mask")) {
      if (ctx.andmask_offset == 0)
        ctx.andmask_offset = off;
      ctx.mainPart->relaDyn->addReloc({ctx.target->andmaskRel, ctx.in.got.get(), off,
        DynamicReloc::AgainstSymbol, sym, 0,
        R_ABS});
        return;
    } else if (sym.getName().starts_with("callback_table") || sym.getName().starts_with("placeholder") || sym.getName().starts_with("dsym_")) {
      // ctx.mainPart->relaDyn->addReloc({ctx.target->callbackRel, ctx.in.got.get(), off,
      //   DynamicReloc::AgainstSymbol, sym, 0,
      //   R_ABS});
      ctx.mainPart->relaDyn->addReloc({ctx.target->customRel, ctx.in.got.get(), off,
        DynamicReloc::AgainstSymbol, sym, 0,
        R_ABS});
        return;
    } else if (sym.customreloc_type() == 4) {
      printf("Name %s\n", sym.getName().str().c_str());
      ctx.mainPart->relaDyn->addReloc({ctx.target->callbackmaybeRel, ctx.in.got.get(), off,
        DynamicReloc::AgainstSymbol, sym, 0,
        R_ABS});
        return;
    } else if (sym.customreloc_type() == 6) {
      ctx.mainPart->relaDyn->addReloc({ctx.target->dsobaseRel, ctx.in.got.get(), off,
        DynamicReloc::AgainstSymbol, sym, 0,
        R_ABS});
        return;
    }
  ...
  else if (sym.getName().starts_with("or_mask")) {
    ctx.mainPart->relaDyn->addReloc({ctx.target->ormaskRel, ctx.in.got.get(), off,
      DynamicReloc::AgainstSymbol, sym, 0,
      R_ABS});
  } else if (sym.getName().starts_with("and_mask")) {
...
void RelocationScanner::processAux(RelExpr expr, RelType type, uint64_t offset,
  ...
  // indirection.
  // if (sym.getName() == "puts" && sym.iscustomreloc) {
  //   printf("YESSSSSSSSSSS %lx\n", type);
  ...
  if (needsGot(expr)) {
    if (ctx.arg.emachine == EM_MIPS) {
      ...
      if (expr == RE_AARCH64_AUTH_GOT || expr == RE_AARCH64_AUTH_GOT_PAGE_PC ||
        ...
        // if (sym.getName() == "puts" && sym.iscustomreloc) {
        //   printf("AAAAAAAAAAA %lx, %lx\n", type, expr);
        // }
        sym.setFlags(NEEDS_GOT | NEEDS_GOT_NONAUTH);
      }
    }
  } else if (needsPlt(expr)) {
    // if (sym.getName() == "puts" && sym.iscustomreloc && sym.hasFlag(NEEDS_GOT)) {
    //   printf("BBBBBBBBB %lx, %lx\n", type, expr);
    // }
  ...
  // if (this->main_bin && sym.getName() == "puts") {
  //   printf("READY TO BREAK IT %lx %lx\n", expr, type);
  //   addRelativeReloc(ctx, *ctx.in.got, offset, mySym, 0, R_ABS, ctx.target->symbolicRel);
  // }
  

  if (isStaticLinkTimeConstant(expr, type, sym, offset) ||
      (!ctx.arg.isPic && sym.isUndefWeak())) {
        // if (this->main_bin && sym.getName() == "or_mask") {
        //   printf("ll Expr: %lx, Type: %lx, %lx\n", expr, type, offset);
        // }
...
void RelocationScanner::scanOne(typename Relocs<RelTy>::const_iterator &i) {
  ...
  // if (this->main_bin && sym.getName() == "sleep") {
  //   //printf("WOOOOOOOOOOOOOOOW\n");
  // } else if (sym.getName() == "sleep") {
  //   printf("Yea of course in %s\n", this->sec->name);
  // }

  /*
  if (this->sec->name == ".init" && sym.isFunc()) {
    sym.iscustomreloc = true;
  }
  */
  if (sym.getName().starts_with("or_mask")) {
        //printf("ORMASK %s\n", sym.getName().str().c_str());
        type = R_X86_64_ORMASK;
        sym.iscustomreloc = 0x2;
  } else if (sym.getName().starts_with("and_mask")) {
    type = R_X86_64_ANDMASK;
    sym.iscustomreloc = 0xa;
  } else if (sym.getName().starts_with("__libc_start_main")) {
    sym.iscustomreloc = 0x1;
  }


  /* HERE WE TAKE CARE OF ALL THE POSSIBLE RELOCATION CASES */
  if (1 > 0 || this->main_bin) {

      if (sym.getName().starts_with("cccallback_table")) {
      //printf("CALLBACK_TABLE\n");
      type = R_X86_64_CALLBACK_TABLE;
      sym.iscustomreloc = 0x3;
    } else if (sym.isPreemptible && sym.isFunc()) {
      //printf("NAME IS %s and type: 0x%lx\n", sym.getName().str().c_str(), type);
      if (type == 0x1) {
        sym.iscustomreloc = 0x1;
        //printf("EXTRA SYMBOL\n");
      } else if (type == 0x29 || type == 0x2a || type == 0x2) {
        sym.iscustomreloc = 0x1;
        //printf("EXTRA SYMBOL\n");
      }
    } else if (sym.getName().ends_with("1337") && sym.getName().contains('_')) {
      
      type = R_X86_64_CALLBACK_MAYBE;
      if (sym.iscustomreloc < 0x100) {
        printf("AAAAAAAAAAAAAAAAA %s 0x%lx\n", sym.getName().str().c_str(), sym.iscustomreloc);
        std::string symName = sym.getName().str();
        size_t name = symName.find('_');
        assert(name != std::string::npos);
        size_t val = symName.find('_', name + 1);
        assert(val  != std::string::npos);
        uint64_t dso_offset = std::strtol((symName.substr(name+1, val-name-1)).c_str(), nullptr, 0);
        assert(dso_offset < 0xffffff);
        sym.iscustomreloc = dso_offset * 256 + 0x4;
        printf("AAAAAAAAAAAAAAAAA %s 0x%lx\n", sym.getName().str().c_str(), sym.iscustomreloc);
        sym.rename(name);
      }
    } else if (sym.getName().starts_with("placeholder") && !sym.getName().starts_with("placeholder_"))  {
      sym.iscustomreloc = 0x1;
    }
  }
  /* END*/
  ...
  if (sym.customreloc_type() > 1 && sym.customreloc_type() != 4) {
    expr = R_GOT_PC;
    //printf("Offset at 0x%lx, type is 0x%lx\n", rel.r_offset, expr);
  }

  // Error if the target symbol is undefined. Symbol index 0 may be used by
  // marker relocations, e.g. R_*_NONE and R_ARM_V4BX. Don't error on them.
  if (sym.isUndefined() && symIndex != 0 && (sym.customreloc_type() < 2) && !sym.getName().ends_with("callback_table") && !sym.getName().starts_with("placeholder") && !sym.getName().starts_with("dsym_") &&
  ...
  if (this->main_bin){
    //printf("Expr is %lx for symbol %s\n", expr, sym.getName());
    //printf("");
  ...
  auto outerFn = [&]() {
    for (ELFFileBase *f : ctx.objectFiles) {
      auto fn = [f, &ctx]() {
        ...
        if (auto *file = dyn_cast<InputFile>(f)) {
          if (file->getName()[0] != '/') {
            scanner.main_bin = true;
          } else if (file->getName()[1] == 't' && file->getName()[2] == 'm') {
            scanner.main_bin = true;
          }
        }
...
void elf::postScanRelocations(Ctx &ctx) {
  auto fn = [&](Symbol &sym) {
    ...
    if (sym.customreloc_type() == 0x1) {
      if (!(flags & NEEDS_PLT)) {
        printf("Wtf in symbol %s\n", sym.getName().str().c_str());
        flags |= NEEDS_PLT;
        tmp = 1;
        sym.flags |= NEEDS_PLT;
      }
    }
    if (!sym.needsDynReloc() && (sym.customreloc_type() < 2)) {
      return;
    }
    sym.allocateAux(ctx);
    ...
    if (sym.customreloc_type() & 0x1) {
      //printf("Creating fake PLT for %s\n", sym.getName());
      addFakePltEntry(ctx, *ctx.in.fakeplt, *ctx.in.gotPlt, *ctx.in.relaPlt,
        ctx.target->pltRel, sym);
    }
    if ((flags & NEEDS_PLT) && tmp == 0 && !sym.getName().starts_with("placeholder")) {
      //printf("Before creating PLT %s\n", sym.getName());
