...
static void demoteDefined(Defined &sym, DenseMap<SectionBase *, size_t> &map) {
  ...
  if (sym.getName().ends_with("__base")) {
    printf("Lemao\n");
  }
...
static bool compareSections(Ctx &ctx, const SectionCommand *aCmd,
  ...
  if (a->name == ".plt" && (b->flags & SHF_EXECINSTR)) {
    return true;
  }
  return false;
}
...
template <class ELFT> void Writer<ELFT>::sortSections() {
  ...
  for (SectionCommand *cmd : ctx.script->sectionCommands) {
    if (auto *osd = dyn_cast<OutputDesc>(cmd)) {
      if (osd->osec.name == ".plt") {
        foundPlt = true;
        //printf("OFFSET IS 0x%lx\n", osd->osec.addralign);
        //osd->osec.addralign = std::max<uint64_t>(osd->osec.addralign, 0x1000);
        continue;
      }
      if (foundPlt) {
        printf("After plt section is: %s\n", osd->osec.name);
        osd->osec.addralign = std::max<uint64_t>(osd->osec.addralign, 0x1000);
        break;
      }
    }
...
template <class ELFT> void Writer<ELFT>::finalizeSections() {
  ...
  if (!ctx.arg.relocatable) {
    ...
    //     printf("PUTS in file %s\n", sym->file->getName());
    //   } else if (sym->getName() == "puts" && sym->iscustomreloc && sym->hasFlag(NEEDS_PLT)) {
    //     printf("LALALALAL in file %s\n", sym->file->getName());
    //   }
    // }
    ...
    // if (ctx.in.relaPlt) {
    //   for (DynamicReloc &reloc : ctx.in.relaPlt->relocs) {
    //     const Symbol &sym = *reloc.sym;
    //     if (sym.iscustomreloc) {
    //       reloc.offsetInSec = sym.getGotPltOffset(ctx);
    //     }
    //     //printf("sdfsf %lx\n", reloc.offsetInSec);
    //   }
    // }
    ...
    if (ctx.in.fakeplt && ctx.in.fakeplt->isNeeded())
      ctx.in.fakeplt->addSymbols();
...

  {
    ...
    for (Symbol *sym : ctx.symtab->getSymbols()) {
      // if (sym->getName() == "_GLOBAL_OFFSET_TABLE_") {
      //   sym->isExported = true;
      //   sym->binding = STB_GLOBAL;
      //   sym->isUsedInRegularObj = true;
      //   sym->isPreemptible = true; 
      //   sym->setVisibility(STV_DEFAULT);
      //   //printf("Exported: 0x%lx\n", sym->isExported);
      // }
...

  {
    ...
    finalizeSynthetic(ctx, ctx.in.fakeplt.get());
...
template <class ELFT> void Writer<ELFT>::assignFileOffsets() {
  ...
  for (OutputSection *sec : ctx.outputSections) {
    ...
    // if (sec->name == ".plt") {
    //   off = 0;
    //   printf("OFFSET FOR PLT IS 0x%lx\n", off);
    // }
    sec->offset = off;
    // if (sec->name == ".lel") {
    //   sec->offset = 7000;
